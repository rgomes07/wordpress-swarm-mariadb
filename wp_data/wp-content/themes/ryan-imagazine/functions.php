...
function ryan_thinkup_child_frontscripts() {

	wp_enqueue_style( 'thinkup-style', get_template_directory_uri() . '/style.css', array( 'thinkup-bootstrap', 'thinkup-shortcodes' ) );
	wp_enqueue_style( 'ryan-thinkup-style-imagazine', get_stylesheet_directory_uri() . '/style.css', array( 'thinkup-style' ), wp_get_theme()->get('Version') );
}
add_action( 'wp_enqueue_scripts', 'ryan_thinkup_child_frontscripts' );
...
function ryan_thinkup_child_adminscripts() {

	if ( is_customize_preview() ) {

		// Add theme stylesheets
		wp_enqueue_style( 'ryan-thinkup-child-backend', get_stylesheet_directory_uri() . '/styles/backend/style-backend.css', '', '' );

	}
}
add_action( 'admin_enqueue_scripts', 'ryan_thinkup_child_adminscripts' );
...
function ryan_thinkup_updateoption_child_settings() {

	// Set theme name combinations
	$name_theme_upper = 'Ryan';
...
add_action( 'init', 'ryan_thinkup_updateoption_child_settings', 999 );

// Remove child theme settings from options array - UPDATED 20180819
function ryan_thinkup_removeoption_child_settings() {
